...
function renderAdminDashboard(data) {
  ...
  const filterHTML = `
    <div class="mb-3 d-flex gap-2 flex-wrap align-items-center">
      ...
      <button class="btn btn-sm ${currentFilter === 'all' ? 'btn-primary' : 'btn-outline-primary'}" onclick="window.filterUsersByLetter('all')">
      ...
      ${letters.map(letter => {
        ...
        return count > 0 ? `
          <button class="btn btn-sm ${currentFilter === letter ? 'btn-primary' : 'btn-outline-secondary'}" onclick="window.filterUsersByLetter('${letter}')">${letter} (${count})</button>
        ` : '';
      }).join('')}
    </div>
  `;
  ...
  const usersHTML = filteredUsers.length === 0 
        ...
        <div class="d-flex justify-content-between align-items-start mb-3">
          <div class="d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <div class="mt-1">
                ${user.calendarConnected ? '<span class="badge bg-success me-1"><i class="bi bi-calendar-check"></i> Calendar</span>' : ''}
                ${user.shiftsCount > 0 ? `<span class="badge bg-info"><i class="bi bi-clock-history"></i> ${user.shiftsCount} shifts</span>` : ''}
              </div>
            </div>
          </div>
          <span class="badge ${user.isOnline ? 'bg-success' : 'bg-secondary'} fs-6">
            ${user.isOnline ? '<i class="bi bi-circle-fill"></i> Online' : '<i class="bi bi-circle"></i> Offline'}
          </span>
        </div>
        ...
        ${user.shifts && user.shifts.length > 0 ? `
          <div class="mt-3">
            <button 
              ...
              aria-expanded="false"
            >
              <i class="bi bi-calendar-event"></i> Ver Shifts (${user.shifts.length})
            </button>
...
function updateAdminStats(data) {
  ...
  const calendarConnected = data.users.filter(u => u.calendarConnected).length;
  const totalShifts = data.users.reduce((sum, u) => sum + (u.shiftsCount || 0), 0);
...
function formatDate(dateString) {
  ...
  try {
    ...
    if (diffDays >= 7 && diffDays < 30) return `${Math.floor(diffDays / 7)} semana(s) atrás`;
